...
if($tab == "products")
{
	...
	// Debug output to check products count
	if($products === false){
...
if($tab == "sales")
{
	...
	$query_total_per_product = "SELECT description, SUM(total) as total_sales FROM sales GROUP BY description ORDER BY total_sales DESC";

	// Query total sales per cashier
	$query_total_per_cashier = "SELECT user_id, SUM(total) as total_sales FROM sales GROUP BY user_id ORDER BY total_sales DESC";
	...
	//if both start and end are set
 	if($startdate && $enddate)
 	{
 		...
 		$query_total_per_product = "SELECT description, SUM(total) as total_sales FROM sales WHERE date BETWEEN '$startdate' AND '$enddate' GROUP BY description ORDER BY total_sales DESC";
 		$query_total_per_cashier = "SELECT user_id, SUM(total) as total_sales FROM sales WHERE date BETWEEN '$startdate' AND '$enddate' GROUP BY user_id ORDER BY total_sales DESC";
	...
	//if only start date is set
 	if($startdate && !$enddate)
 	{
 		...
 		$query_total_per_product = "SELECT description, SUM(total) as total_sales FROM sales WHERE date = '$startdate' GROUP BY description ORDER BY total_sales DESC";
 		$query_total_per_cashier = "SELECT user_id, SUM(total) as total_sales FROM sales WHERE date = '$startdate' GROUP BY user_id ORDER BY total_sales DESC";
 	}
	...
	$sales_total_per_product = $saleClass->query($query_total_per_product);

	$sales_total_per_cashier = $saleClass->query($query_total_per_cashier);
...
if($tab == "users")
{
	...
	// Debug output to check users count
	if($users === false){
...
if($tab == "dashboard")
{
	...
	// Query total sales per product for dashboard
	$query_total_per_product = "SELECT description, SUM(total) as total_sales FROM sales GROUP BY description ORDER BY total_sales DESC";
	$total_sales_per_product = $db->query($query_total_per_product);
